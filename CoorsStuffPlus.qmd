...
```{r}
# Loop through baseball savant search for each day needed
...
```{r}
# Convert data to the prefered units
data_prep <- function(df) {
  ...
  df <- df %>%
    ...
    mutate(
      # Find the average and max velocity of the pitchers fastballs
      ...
      # Find the average and max ivb of the pitchers fastballs
...
```{r}
# Take baseball savants pitch outcomes and convert to simple identifiers
...
# Find the mean run value based off the count and pitch outcome  
mean_rv_df <- df %>%
...
```{r}
# Filter out pitch types used by position players 
...
```{r}
# Select only needed columns from the data
...
# Used suggested paramters for model 
params <- list(
...
# Check the importance of the features to the model
...
```{r}
# Use the model to grade more test set
...
```{r}
# Save the run values for future pitch grading. 
stuff_model = xgb.load("general_stuff_modelv6.model")
training_data = read_csv_arrow("~/training_data2.csv")
...
stuff_grading = function(df, model_path="general_stuff_modelv6.model", year = 2024){
  ...
  print(names(df))
  # Use predictions to grade the new pitches 
  predictions = predict(load_model, grading_data)
  stuff_score = -predictions
  # Scale the data so an average pitch is 100 and one SD is 20 points of stuff+ 
...
```{r}
# Grade pitchers by their pitches stuff grades and total stuff grade
...
```{r}
# Compare stuff+ against pitching stats
...
```{r}
# Now that the stuff+ proccess has some validity for predicting future outcomes repeat with just coors field data only 
